...
C {code_shown.sym} 350 -520 0 0 {name=spice only_toplevel=false value=
"
.control
dc VIN_2 -2.5 2.5 10m

let loop_gain=deriv(v(vout))
meas dc swing_min find v(vout) when loop_gain=0.9 rise=1
meas dc swing_max find v(vout) when loop_gain=0.9 fall=1

write tb_opamp_dc.raw
.endc
"}
C {vsource.sym} 300 -450 0 0 {name=VIN_2 value=0 savecurrent=false}
